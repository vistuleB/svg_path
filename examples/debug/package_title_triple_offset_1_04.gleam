//// Full package-title first through third offset probe at distance 1.04.
...
const output = "examples/debug/package_title_triple_offset_1_04.svg"

const offset_distance = 1.04
...
pub fn main() -> Nil {
  ...
  case third_offset {
    Ok(third_offset) ->
      io.println(
        "third offset subpaths: "
        <> int.to_string(list.length(svg_path.path_subpaths(third_offset))),
      )
    Error(error) -> io.println("third offset error: " <> string.inspect(error))
  }

  write_file(output, render(source, first_offset, second_offset, third_offset))
  Nil
}
...
fn render(
  ...
  let boxes = case third_offset {
    Ok(third_offset) ->
      path_boxes([source, first_offset, second_offset, third_offset])
    Error(_) -> path_boxes([source, first_offset, second_offset])
  }
  let view_box = padded_box(boxes, margin: 2.0)
  let things = case third_offset {
    Ok(third_offset) -> [
      background(view_box),
      ...
      svg.StyledPath(
        second_offset,
        "fill: none; stroke: #dc2626; stroke-width: 0.08; stroke-linecap: round; stroke-linejoin: round",
      ),
      svg.StyledPath(
        third_offset,
        "fill: none; stroke: #16a34a; stroke-width: 0.08; stroke-linecap: round; stroke-linejoin: round",
      ),
    ]
    Error(error) -> [
      background(view_box),
      ...
      svg.Text(
        "third offset error: " <> string.inspect(error),
        "fill: #dc2626; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace",
        svg_path.Point(view_box.min.x +. 1.0, view_box.min.y +. 1.0),
        0.45,
      ),
    ]
  }
