...
main(_) ->
    ...
    lists:foreach(fun({Label,Curve})->
      lists:foreach(fun({Solver,Limit})->
        {Micros,Result}=timer:tc(fun()->'svg_path@intersections':experimental_curve_intersections(
          Curve,Flat,Solver,'svg_path@intersections':default_options(),Limit) end),
        Summary=case Result of
          {ok,Hits}->{ok,length(Hits),[{T,U}||{segment_intersection,T,U,_}<-Hits]};
          Error->Error
        end,
        io:format("~s ~p budget=~p time=~pms: ~p~n",[Label,Solver,Limit,Micros div 1000,Summary])
      end,[{henry,1000},{edward,1000},{elizabeth,1000},{elizabeth,100000}])
