//// Parser cases adapted from the W3C SVG 1.1 Second Edition Test Suite.
////
//// Original suite:
//// https://www.w3.org/Graphics/SVG/Test/20110816/
////
//// The original fixtures are rendering comparisons. These adaptations retain
//// their path-data strings and identifiers, but test only parsing semantics.
...
// W3C SVG 1.1: paths-data-01-t.svg — cubic C/c/S/s commands.
pub fn w3c_svg11_paths_data_01_test() {
  assert_roundtrips([
...
// W3C SVG 1.1: paths-data-02-t.svg — quadratic Q/q/T/t commands.
pub fn w3c_svg11_paths_data_02_test() {
  assert_roundtrips([
...
// W3C SVG 1.1: paths-data-03-f.svg — elliptical A/a commands.
pub fn w3c_svg11_paths_data_03_test() {
  assert_roundtrips([
...
// W3C SVG 1.1: paths-data-04-t.svg through paths-data-10-t.svg —
// absolute, relative, implicit, horizontal, vertical, and close commands.
pub fn w3c_svg11_paths_data_04_through_10_test() {
  assert_roundtrips([
...
// W3C SVG 1.1: paths-data-12-t.svg through paths-data-16-t.svg —
// smooth controls, repeated arguments, and implicit lineto after moveto.
pub fn w3c_svg11_paths_data_12_through_16_test() {
  assert_roundtrips([
...
// W3C SVG 1.1: paths-data-17-f.svg — Z and z are equivalent.
pub fn w3c_svg11_paths_data_17_test() {
  assert_equivalent(
...
// W3C SVG 1.1: paths-data-18-f.svg — whitespace, maximal number
// consumption, and invalid trailing input. Browsers retain the valid prefix;
// this strict Result parser rejects the entire invalid string.
pub fn w3c_svg11_paths_data_18_test() {
  assert_equivalent("M 20 40\nH 40", "M 20 40 H 40")
  assert_equivalent("\nM\n20\n60\nH\n40\n", "M 20 60 H 40")
  assert_equivalent("M 20 80 H40", "M 20,80 H 40")
  assert_equivalent("M 20 120 H 40.5 0.6", "M 20 120 H 40.5.6")
  assert_equivalent("M 20 140 h 10 -20", "M 20 140 h 10-20")

  let assert Error(_) = parse.path("M 20 100 H 40#90")
  let assert Error(_) = parse.path("M 20 160 H 40#90")
}

// W3C SVG 1.1: paths-data-19-f.svg — repeated argument groups may omit
// repeated command letters.
pub fn w3c_svg11_paths_data_19_test() {
  [
    #("M20 20 H40 H60", "M20 20 H40 60"),
    #("M20 40 h20 h20", "M20 40 h20 20"),
    #("M120 20 V40 V60", "M120 20 V40 60"),
    #("M140 20 v20 v20", "M140 20 v20 20"),
    #("M220 20 L240 20 L260 20", "M220 20 L240 20 260 20"),
    #("M220 40 l20 0 l20 0", "M220 40 l20 0 20 0"),
    #(
      "M50 150 C50 50 200 50 200 150 C200 50 350 50 350 150",
      "M50 150 C50 50 200 50 200 150 200 50 350 50 350 150",
    ),
    #(
      "M50 250 S125 200 200 250 S275 200 350 250",
      "M50 250 S125 200 200 250 275 200 350 250",
    ),
    #(
      "M50 300 Q125 275 200 300 Q275 325 350 300",
      "M50 300 Q125 275 200 300 275 325 350 300",
    ),
    #("M425 25 T425 75 T425 125", "M425 25 T425 75 425 125"),
    #(
      "M400 200 A25 25 0 0 0 425 150 A25 25 0 0 0 400 200",
      "M400 200 A25 25 0 0 0 425 150 25 25 0 0 0 400 200",
    ),
  ]
  |> list.each(fn(pair) {
    let #(explicit, implicit) = pair
    assert_equivalent(explicit, implicit)
  })
}

// W3C SVG 1.1: paths-data-20-f.svg — compact arc flags and invalid flags.
pub fn w3c_svg11_paths_data_20_test() {
  assert_equivalent(
    "M120,120 h25 a25,25 0 1,0 -25,25 z",
    "M120,120 h25 a25,25 0 10 -25,25z",
  )
  assert_equivalent(
    "M200,120 h-25 a25,25 0 1,1 25,25 z",
    "M200,120 h-25 a25,25 0 1125,25 z",
  )
  assert_equivalent(
    "M120,200 h25 a25,25 0 1,1 -25,-25 z",
    "M120,200 h25 a25,25 0 1 1-25,-25 z",
  )
...
fn assert_roundtrips(sources: List(String)) -> Nil {
  sources
  |> list.each(fn(source) {
    let assert Ok(path) = parse.path(source)
    let assert Ok(_) = path |> serialize.path |> parse.path
  })
}

fn assert_equivalent(first: String, second: String) -> Nil {
  let assert Ok(first_path) = parse.path(first)
  let assert Ok(second_path) = parse.path(second)
