...
fn print_pair_bounding_boxes(left_index: Int, right_index: Int) -> Nil {
  let assert Ok(provisional) =
    debug_provisional_with(smooth_figure_eight(), offset.Round, -16.0)
  let segments = svg_path.segments(provisional)
  let assert Ok(left) = nth(segments, left_index)
  let assert Ok(right) = nth(segments, right_index)
  let assert Ok(left_box) = svg_path.segment_bounding_box(left)
  let assert Ok(right_box) = svg_path.segment_bounding_box(right)
  io.println("")
  io.println(
    "pair " <> int.to_string(left_index) <> "/" <> int.to_string(right_index),
  )
  io.println("left kind=" <> segment_kind(left))
  print_segment_details("left segment", left)
  print_box("left box", left_box)
  print_point("left end", svg_path.segment_end(left))
  io.println("right kind=" <> segment_kind(right))
  print_box("right box", right_box)
  print_point("right start", svg_path.segment_start(right))
  io.println(
    "endpoint distance="
    <> f(distance(svg_path.segment_end(left), svg_path.segment_start(right))),
  )
}

fn print_segment_details(label: String, segment: svg_path.Segment) -> Nil {
  io.println(label)
  print_point("  start", svg_path.segment_start(segment))
  print_point("  end", svg_path.segment_end(segment))
  case segment {
    svg_path.CubicBezier(control1:, control2:, ..) -> {
      print_point("  control1", control1)
      print_point("  control2", control2)
    }
    _ -> Nil
  }
}

fn print_box(label: String, box: svg_path.BoundingBox) -> Nil {
  io.println(
    label
    <> " min=("
    <> f(box.min.x)
...
fn print_point(label: String, point: svg_path.Point) -> Nil {
  io.println(label <> "=(" <> f(point.x) <> ", " <> f(point.y) <> ")")
}

fn print_minimized_intersection_comparison() -> Nil {
  let assert Ok(provisional) =
    debug_provisional_with(smooth_figure_eight(), offset.Round, -16.0)
  let segments = svg_path.segments(provisional)
  io.println("")
  io.println("figure-eight adjacent chord vs minimizer t-pairs")
  [67, 68, 69, 70, 71, 72, 73, 74]
  |> list.map(fn(left_index) {
    print_minimized_pair(segments, left_index, left_index + 1)
  })
  Nil
}

fn print_minimized_pair(
  ...
  let assert Ok(left) = nth(segments, left_index)
  let assert Ok(right) = nth(segments, right_index)
  let assert Ok([intersection]) = svg_path.segment_intersections(left, right)
  let minimized =
    minimize_segment_distance(
      left,
      right,
      intersection.left_t,
  ...
  let #(min_left_t, min_right_t, min_distance) = minimized
  let chord_distance =
    segment_pair_distance(
      left,
      intersection.left_t,
      right,
      intersection.right_t,
  ...
  io.println(
    int.to_string(left_index)
    <> "/"
    <> int.to_string(right_index)
    <> " chord=("
...
fn minimize_segment_distance(
  ...
  let distance = segment_pair_distance(left, left_t, right, right_t)
  minimize_segment_distance_loop(
    left,
    right,
    left_t,
    right_t,
...
fn minimize_segment_distance_loop(
  left: svg_path.Segment,
  right: svg_path.Segment,
  left_t: Float,
  right_t: Float,
  ...
  case iterations <= 0 || step <=. 0.000000000001 {
    True -> #(left_t, right_t, best_distance)
    False -> {
      let #(next_left_t, next_right_t, next_distance) =
        best_neighbor(left, right, left_t, right_t, best_distance, step)
      case next_distance <. best_distance {
        True ->
          minimize_segment_distance_loop(
            left,
            right,
...
fn best_neighbor(
  ...
  |> list.fold(#(left_t, right_t, best_distance), fn(best, candidate) {
    let #(candidate_left_t, candidate_right_t) = candidate
    let candidate_left_t = clamp01(candidate_left_t)
    let candidate_right_t = clamp01(candidate_right_t)
    let candidate_distance =
      segment_pair_distance(left, candidate_left_t, right, candidate_right_t)
    case candidate_distance <. best.2 {
      True -> #(candidate_left_t, candidate_right_t, candidate_distance)
      False -> best
    }
  })
}

fn segment_pair_distance(
  left: svg_path.Segment,
  left_t: Float,
  right: svg_path.Segment,
  right_t: Float,
) -> Float {
  let assert Ok(left_point) = svg_path.segment_point(left, at: left_t)
  let assert Ok(right_point) = svg_path.segment_point(right, at: right_t)
...
fn print_parametric_cut_diagnostics(
  ...
  let assert Ok(result) =
    offset.subpath_parametric_with(source, distance:, options:)
  let segments = svg_path.segments(provisional)
  let cuts =
...
fn print_diamond_round_negative_distances() -> Nil {
  ...
  let assert Ok(result) =
    offset.subpath_parametric_with(source, distance: offset_distance, options:)
  let threshold =
  ...
  |> list.flat_map(fn(subpath) {
    ...
    |> list.map(fn(t) {
      ...
      svg.Circle(
        point,
        0.9,
        "fill: " <> color <> "; stroke: white; stroke-width: 0.25",
      )
    })
  })
}
...
fn render_panel(
  ...
  let assert Ok(result) =
    offset.subpath_parametric_with(
      example.source,
...
fn debug_provisional_with(
  ...
  case join {
    offset.Round -> {
      let assert Ok(subpath) = offset.subpath_with(source, distance:, options:)
      Ok(subpath)
    }
    _ -> {
      let pieces =
...
fn split_indexed_segments(
  segments: List(DebugIndexedSegment),
  all: List(DebugIndexedSegment),
  total total: Int,
  current current: List(svg_path.Segment),
  sections sections: List(List(svg_path.Segment)),
) -> List(List(svg_path.Segment)) {
  case segments {
    ...
    [first, ..rest] -> {
      let parameters =
        split_parameters(first, all, total:, parameters: [
          DebugSplitParameter(0.0, False),
          DebugSplitParameter(1.0, False),
        ])
      let pieces = split_piece(first.segment, parameters)
      let #(current, sections) =
        append_split_pieces(pieces, current:, sections:)
      split_indexed_segments(rest, all, total:, current:, sections:)
    }
  }
}

fn split_parameters(
  indexed: DebugIndexedSegment,
  all: List(DebugIndexedSegment),
  total total: Int,
  parameters parameters: List(DebugSplitParameter),
) -> List(DebugSplitParameter) {
  let DebugIndexedSegment(index:, segment: left) = indexed
  case all {
    [] ->
      parameters
      |> list.sort(by: fn(a, b) {
      ...
      |> unique_split_parameters([])
    [DebugIndexedSegment(index: other_index, segment: right), ..rest] -> {
      let parameters = case other_index == index {
        True -> parameters
        False -> {
          let assert Ok(intersections) =
            svg_path.segment_intersections(left, right)
          intersections
          |> list.fold(parameters, fn(parameters, intersection) {
            case
              adjacent_endpoint_intersection(
                intersection,
                left,
              ...
              )
            {
              True -> parameters
              False -> [
                DebugSplitParameter(clamp01(intersection.left_t), True),
                ..parameters
              ]
            }
          })
      ...
      split_parameters(indexed, rest, total:, parameters:)
    }
  }
}
...
fn path_arrows(path: svg_path.Path, color: String) -> svg.ThingsToDraw {
  ...
  |> svg_path.subpaths
  |> list.flat_map(fn(subpath) {
    case longest_segment(svg_path.segments(subpath), Error(Nil)) {
      Error(_) -> []
      Ok(segment) -> {
        let assert Ok(point) = svg_path.segment_point(segment, at: 0.45)
        let direction = segment_direction(segment)
        case normalize(direction) {
          Error(_) -> []
          Ok(unit) -> [arrow(point, unit, color)]
        }
      }
    }
  })
...
fn longest_segment(
  segments: List(svg_path.Segment),
  best: Result(#(svg_path.Segment, Float), Nil),
  ...
  case segments {
    ...
    [segment, ..rest] -> {
      ...
      let best = case best {
        ...
        Ok(#(_, current)) if length >. current -> Ok(#(segment, length))
        Ok(_) -> best
      }
