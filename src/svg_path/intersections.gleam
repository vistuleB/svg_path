...
fn window_preserving_initial_windows(
  count count: Int,
  depth depth: Int,
) -> List(#(WindowPreservingWindow, Int)) {
  let intervals =
    window_preserving_parameter_intervals(0, count:, accumulated: [])
  list.fold(intervals, [], fn(windows, left_interval) {
    ...
    list.fold(intervals, windows, fn(windows, right_interval) {
      ...
      [
        #(
          WindowPreservingWindow(left_from, left_to, right_from, right_to),
          depth,
        ),
        ..windows
      ]
...
fn window_preserving_parameter_intervals(
  ...
  case index >= count {
    ...
    False -> {
      ...
      window_preserving_parameter_intervals(index + 1, count:, accumulated: [
        #(from, to),
        ..accumulated
      ])
    }
...
fn window_preserving_chord_closest_parameters(
  ...
  let wx = p.x -. q.x
  let wy = p.y -. q.y
  let a = window_preserving_dot(ux, uy, ux, uy)
  let b = window_preserving_dot(ux, uy, vx, vy)
  let c = window_preserving_dot(vx, vy, vx, vy)
  let d = window_preserving_dot(ux, uy, wx, wy)
...
fn window_preserving_dot(ax: Float, ay: Float, bx: Float, by: Float) -> Float {
  ax *. bx +. ay *. by
}

fn window_preserving_chord_crossing(
  p: svg_path.Point,
  p2: svg_path.Point,
  q: svg_path.Point,
  q2: svg_path.Point,
  ...
  case directions_are_independent(Point(rx, ry), Point(sx, sy)) {
    ...
    True -> {
      ...
      let right_t = window_preserving_cross(qpx, qpy, rx, ry) /. denominator
      case
        window_preserving_inside01(left_t)
        && window_preserving_inside01(right_t)
      {
        True ->
          Some(#(
            window_preserving_clamp01(left_t),
            window_preserving_clamp01(right_t),
          ))
        False -> None
      }
    }
...
fn window_preserving_split_window_nine(
  window: WindowPreservingWindow,
) -> List(WindowPreservingWindow) {
  let WindowPreservingWindow(left_from:, left_to:, right_from:, right_to:) =
    window
  let left_a = window_preserving_interpolate(left_from, left_to, 1.0 /. 3.0)
  let left_b = window_preserving_interpolate(left_from, left_to, 2.0 /. 3.0)
  let right_a = window_preserving_interpolate(right_from, right_to, 1.0 /. 3.0)
  let right_b = window_preserving_interpolate(right_from, right_to, 2.0 /. 3.0)
  [
    WindowPreservingWindow(left_from, left_a, right_from, right_a),
    WindowPreservingWindow(left_from, left_a, right_a, right_b),
    WindowPreservingWindow(left_from, left_a, right_b, right_to),
    WindowPreservingWindow(left_a, left_b, right_from, right_a),
    WindowPreservingWindow(left_a, left_b, right_a, right_b),
    WindowPreservingWindow(left_a, left_b, right_b, right_to),
    WindowPreservingWindow(left_b, left_to, right_from, right_a),
...
fn window_preserving_interpolate(from: Float, to: Float, t: Float) -> Float {
  from +. { to -. from } *. t
}

fn window_preserving_cross(
  ax: Float,
  ay: Float,
  bx: Float,
  by: Float,
...
fn window_preserving_inside01(value: Float) -> Bool {
  value >=. -0.000000000001 && value <=. 1.000000000001
}

fn window_preserving_clamp01(value: Float) -> Float {
...
// Elizabeth's shared terminal refinement. Chords give one seed, not an intersection
