...
Run `node scripts/gallery/w3c_join_comparison.mjs` after building
`examples/public_api_smoke` with `gleam build --target javascript`.
The script uses public `stroke.subpath` calls for every blue outline.
